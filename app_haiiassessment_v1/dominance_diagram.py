...
def dominance_diagram(logodds, stds, title, inverted=False, cut=False, filename=None):
    ...
    for s in logodds.index:
        ...
        for v in logodds.columns:
            ...
            if cut:
                plt.xlim(0.1,10)
                plt.xticks([0.1, 0.2, 0.5, 1, 2, 5, 10], [0.1, 0.2, 0.5, "No impact", 2, 5, 10])
            else:
                plt.xlim(0.001,1000)
                plt.xticks([0.001,0.01,0.1, 1, 10, 100, 1000], [0.001, 0.01, 0.1, "No impact", 10, 100, 1000])
    ...
    for s in logodds.index:
        ...
        for v in logodds.columns:
            ...
            # Controllo di soglia anche per la legenda
            if np.isinf(val) or val > 1e3:
                label_text = f"inf"
            else:
                label_text = f"{str(v)} {val:.2f} [{val-lower:.2f}, {val+upper:.2f}]"

            handles.append(Line2D([0], [0],marker='o',color=palette[i],label=label_text + st,markeredgecolor='k'))
            i += 1

    plt.legend(handles=handles)
    id = str(uuid.uuid4())
    # Crea il nome del file con il formato corretto
    filename = "output2/" + id + "dominance_" + title + ".png"
...
def compute_reliance(data):
    ...
    for type_ai in data["Type_AI"].unique():

        colname = "Count (" + type_ai + ")"
        sum_ai = patterns[colname].iloc[:n].sum()

        hd1_percentage = safe_divide(patterns.loc[patterns["HD1"]==1, colname].sum(), sum_ai)
        ai_percentage = safe_divide(patterns.loc[patterns["AI"]==1, colname].sum(), sum_ai)
        fhd_percentage = safe_divide(patterns.loc[patterns["FHD"]==1, colname].sum(), sum_ai)

        patterns = add_patterns_percentage(patterns, colname, sum_ai, n)

        patterns.loc[n, colname] = f"tot: {sum_ai}"
        patterns.loc[len(patterns.index)] = ""
        patterns.loc[len(patterns.index)-1, "HD1"] = f"{hd1_percentage:.2%}"
        if ("AI" in data.columns):
            patterns.loc[len(patterns.index)-1, "AI"] = f"{ai_percentage:.2%}"
        patterns.loc[len(patterns.index)-1, "FHD"] = f"{fhd_percentage:.2%}"

        patterns.loc[len(patterns.index)-1, colname] = type_ai


    return patterns


def add_patterns_percentage(patterns, colname, sum_ai, n):

    for idx in range(n):
        ai = patterns.at[idx, colname]
        percentage = ai * 100 / sum_ai
        if percentage < 10:
            formatted_percentage = "{:.2f}%".format(percentage)
        else:
            formatted_percentage = "{:.1f}%".format(percentage)
        patterns.at[idx, colname] = str(ai) + " (" + formatted_percentage + ")"
...
def compute_dominance(data, cut=False, filename=None):
    ...
    for s in data["Study"].unique():
        ...
        for v in study["Type_AI"].unique():
            ...
            epsilon = 1e-10
            logodds_general.loc[s,v] = np.log(safe_divide(cer.loc[s,v] + epsilon, (1-cer.loc[s,v]) + epsilon) *
                                  safe_divide((1-aier.loc[s,v]) + epsilon, aier.loc[s,v] + epsilon))

            stds_general.loc[s,v] = safe_divide(1, aier.loc[s,v] * data_temp.shape[0])
            stds_general.loc[s,v] += safe_divide(1, cer.loc[s,v] * data_temp.shape[0])
            stds_general.loc[s,v] += safe_divide(1, (1 - aier.loc[s,v]) * data_temp.shape[0])
            stds_general.loc[s,v] += safe_divide(1, (1 - cer.loc[s,v]) * data_temp.shape[0])
            ...
            if ("AI" in data.columns):
                ...
                #epsilon = 1e-10
                #if 'res_ab' not in locals():
                #    res_ab = np.array([epsilon, epsilon], dtype=np.float64)  # Inizializza con piccoli valori

                #if 'res_av' not in locals():
                #    res_av = np.array([epsilon, epsilon], dtype=np.float64)  # Inizializza con piccoli valori

                #logodds_ab.loc[s,v] = np.log(0.001 + safe_divide(res_ab[0] + epsilon, (1 - res_ab[0]) + epsilon) * safe_divide((1 - res_ab[1]) + epsilon, res_ab[1] + epsilon))
                #logodds_av.loc[s,v] = np.log(0.001 + safe_divide(res_av[0] + epsilon, (1 - res_av[0]) + epsilon) * safe_divide((1 - res_av[1]) + epsilon, res_av[1] + epsilon))
    ...
    return diagrams, reliance, cer, aier, logodds_general, logodds_ab, logodds_av, stds_general, stds_ab, stds_av
...
def compute_causal_dominance(data, cut=False, filename=None):
    ...
    return diagrams, logodds_ab, logodds_av, stds_ab, stds_av
...
def compute_chi_diagrams(data, cut=False, causal=True, filename=""):
    ...
    diagrams, reliance, cer, aier, logodds_general, logodds_ab, logodds_av, stds_general, stds_ab, stds_av = compute_dominance(data, cut=cut, filename=filename)

    if "Complexity" in data.columns:
        logodds_ab_causal, logodds_av_causal, stds_ab_causal, stds_av_causal = 0, 0, 0, 0
        if ("AI" in data.columns):
            diagramsCaus, logodds_ab_causal, logodds_av_causal, stds_ab_causal, stds_av_causal = compute_causal_dominance(data, cut=cut, filename=filename)
            diagrams = diagrams + diagramsCaus
    ...
    for s in reliance.keys():
        id = str(uuid.uuid4())
        filename = "user-generated/" + id + "reliance_" + s + ".html"
