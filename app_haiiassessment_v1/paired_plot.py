...
def paired_plot(all_data, type_ai=None, group_user=None, sub=None, sub_vals=[], ai_level=None, savename="plot", palette=None, measure="Accuracy"):
    ...
    for s in all_data["Study"].unique():
        for t in all_data[all_data["Study"] == s]["Type_AI"].unique():
            ...
            if ai_level is not None:
                lifted = diff[(diff > 0) & (acc_post < ai_level)].shape[0]/diff.shape[0]
                repulsed = diff[(diff < 0) & (acc_pre < ai_level)].shape[0]/diff.shape[0]
                outperformers = diff[(diff > 0) & (acc_post >= ai_level) & (acc_pre < ai_level)].shape[0]/diff.shape[0]
                ballasted = diff[(diff < 0) & (acc_pre >= ai_level) & (acc_post >= ai_level)].shape[0]/diff.shape[0]
                spurred = diff[(diff > 0) & (acc_pre >= ai_level)].shape[0]/diff.shape[0]
                overcome = diff[(diff < 0) & (acc_pre >= ai_level) & (acc_post < ai_level)].shape[0]/diff.shape[0]
                ...
                annot = f'Lifted: {lifted:.2f},   Repulsed: {repulsed:.2f}, Overcome: {overcome:.2f},\nOutperformers: {outperformers:.2f}, Ballasted: {ballasted:.2f}, Spurred: {spurred:.2f}\nEmpowered: {empowered:.2f}, Unaffected: {unaffected:.2f}, Undermined: {undermined:.2f}'
            else:
                empowered = diff[(diff > 0)].shape[0]/diff.shape[0]
                undermined = diff[(diff < 0)].shape[0]/diff.shape[0]
                unaffected = diff[(diff == 0)].shape[0]/diff.shape[0]
            ...
            if loc_group_user is not None:
                ...
                for i, v in enumerate(np.unique(data[loc_group_user])):
                    ...
                    if v == "Others":
                        ax1.scatter(x1, acc_pre_v, color="green", alpha=0.7, marker='o', label=v + " (n=" + str(group_size) + ")")
                        ax1.scatter(x2, acc_post_v, color="green", alpha=0.7, marker='o')
                    else:
                    ...
                    if v == "Others":
                        ax2.errorbar(x=np.random.normal(loc=0.0, scale=0.1), y=mean_diff_v, xerr=ci_v, fmt='o', color="green", capsize=5)
                        #out = out + str(v) + ": " + str(round(mean_diff_v,3)) + " (" + str(ci_v) + ")\n "
                    else:
