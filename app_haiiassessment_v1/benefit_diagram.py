...
def compute_benefits(data, type_ai=None, group_var=None, group_vals=[], savename="plot", palette=None, measure="Accuracy"):
    ...
    for s in grouped["Study"].unique():
        for v in grouped[grouped["Study"] == s]["Type_AI"].unique():
            ...
            diagrams.append(benefit_diagram(baseline, difference, temp["Type_H"] if "Type_H" in temp.columns else temp["Type_H2"], name, palette, measure, str(s) + " (" + str(v) + ")"))
...
def benefit_diagram(baseline, difference, groups, ai_group="", palette=None, measure="Accuracy", name=""):
    ...
    id = str(uuid.uuid4())
    filename = "output1/" + id + "benefit_" + ai_group + ".png"
